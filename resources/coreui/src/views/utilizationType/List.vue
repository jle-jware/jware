				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-12 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ name: 'Add Job Status' }" class="btn btn-link">
					...
					<table class="table table-striped">
						...
						<tbody>
							<tr v-for="jobStat in jobStats" v-bind:key="jobStat.id">
								<td>
									<router-link :to="{ path: `/jobStat/${jobStat.id}`}">
										<i class="fa fa-edit fa-lg" title="Edit Job Status" style="font-size:20px;  color: black;"></i>
									</router-link>

									<router-link :to="{ path:`/jobStats`}">
										<span v-on:click="erase(jobStat.id)">
											<i class="fa fa-trash fa-lg" title="Delete Job Status" style="font-size:20px;  color: black;"></i>
										</span>
									</router-link>
								</td>
								<td>
		...
		methods: {
			...
			erase(id) {
				...
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/jobStat/${id}`, {
							headers: {
								Authorization: `Bearer ${this.$store.state.token}`
							}
						})
