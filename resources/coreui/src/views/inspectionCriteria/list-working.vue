				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/inspectionCriteria/create' }" class="btn btn-link">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(inspectionCriteria, index) in sortedPriority" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="inspectionCriteria.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkColumnVisibilityStatus('name')">
											<router-link :to="{ path: `/inspectionCriteria/${inspectionCriteria.id}`}">
												{{ inspectionCriteria.name }}
											</router-link>
										</td>
									</tr>
								</tbody>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				this.$swal('inspectionCriteria Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/inspectionCriteria/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedPriority.forEach(inspectionCriteria => {
							self.selectedIds.push(inspectionCriteria.id.toString())
						})
					}
					else {
			...
			load(page) {
				NProgress.start()
				let url = `/api/inspectionCriterias-datatable`
				...
				this.$http.get(url + params,
					...
					.then(({ data }) => {
						console.log('inspectionCriterias', data)
						this.inspectionCriterias = data.data
		...
		computed: {
			filteredPriority: function () {
				var self = this
				return this.inspectionCriterias.filter(function (inspectionCriteria) {
					return inspectionCriteria.name ? inspectionCriteria.name.toLowerCase().indexOf(self.search.name.toLowerCase()) >= 0 : true
				})	
			},
