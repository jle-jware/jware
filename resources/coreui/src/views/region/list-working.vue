				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/region/create' }" class="btn btn-link">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(region, index) in sortedPriority" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="region.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkColumnVisibilityStatus('name')">
											<router-link :to="{ path: `/region/${region.id}`}">
												{{ region.name }}
											</router-link>
										</td>
									</tr>
		...
		methods: {
			...
			erase() {
				...
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/region/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedPriority.forEach(region => {
							self.selectedIds.push(region.id.toString())
						})
					}
					else {
		...
		computed: {
			filteredPriority: function () {
				...
				return this.regions.filter(function (region) {
					return region.name ? region.name.toLowerCase().indexOf(self.search.name.toLowerCase()) >= 0 : true
				})	
			},
