				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-12 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ name: 'Add Job Type' }" class="btn btn-link">
							...
							<tr v-for="jobType in jobTypes" v-bind:key="jobType.id">
								<td>
									<router-link :to="{ path: `/jobType/${jobType.id}`}">
										<i class="fa fa-edit fa-lg" title="Edit Job Type" style="font-size:20px;  color: black;"></i>
									</router-link>

									<router-link :to="{ path:`/jobTypes`}">
										<span v-on:click="erase(jobType.id)">
											<i class="fa fa-trash fa-lg" title="Delete Job Type" style="font-size:20px;  color: black;"></i>
										</span>
									</router-link>
								</td>
								<td>
		...
		methods: {
			...
			erase(id) {
				...
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/jobType/${id}`, {
							headers: {
								Authorization: `Bearer ${this.$store.state.token}`
							}
						})
