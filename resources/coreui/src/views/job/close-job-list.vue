				...
				<b-card header="" border-variant="light">
					...
					<div class="row">
						...
						<div class="col-md-7 col-sm-12 text-right header-menu-nav">
							...
							<!-- <router-link v-if="$store.state.user.contact_type != 'Technician'" :to="{ path: '/job/new/create' }" class="btn btn-link">
								<i class="fa fa-plus-square icon" title="Add Item"></i>
							</router-link> -->
							<router-link :to="{ path: '/jobs' }" class="btn btn-link">
								<i class="fas fa-folder-open icon" title="See Open Job List"></i>
							</router-link>
							<button v-if="$store.state.user.contact_type != 'Technician'" :disabled="!selectedIds.length" class="btn btn-link" @click.prevent="erase" title="Delete Item">
							...
							<table class="table table-striped table-responsive">
								...
								<tbody>
									<tr v-for="(job, index) in sortedjob" :key="index">
										...
										<td v-if="checkColumnVisibilityStatus('contractor')">{{ job.contractor ? job.contractor:'' }}</td>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				columns: [
					...
					{name : 'contractor', label : 'Contractor Name'},
		...
		methods: {
			...
			load(page) {
				NProgress.start()
				let url = `/api/job-list-by-job-status`
