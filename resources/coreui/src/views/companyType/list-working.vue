				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/companyType/create' }" class="btn btn-link">
							...
							<download-excel :data="sortedcompanyType" class="btn">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(companyType, index) in sortedcompanyType" :key="index">
										<td v-if="checkSelectedColumn('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="companyType.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkSelectedColumn('type')">
											<router-link :to="{ path: `/companyType/${companyType.id}`}">
												{{ companyType.type }}
											</router-link>
										</td>
										<td v-if="checkSelectedColumn('comment')">{{ companyType.comment }}</td>
									</tr>
								</tbody>
								<!-- Table Body End -->
							</table>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				// Use sweetalret2
				this.$swal('companyType Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/companyType/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedcompanyType.forEach(companyType => {
							self.selectedIds.push(companyType.id.toString())
						})
					}
					else {
						if (self.sortedcompanyType.length == self.selectedIds.length) {
							self.selectedIds = []
						}
					}
				})
			...
			load(page) {
				let url = `/api/companyTypes?page=${page}&per_page=${this.pagination.per_page}&search=${this.search.global}`
				this.$http.get(url,
					{
						headers: {
							Authorization: `Bearer ${this.$store.state.token}`
		...
		computed: {
			filteredcompanyType: function () {
				var self = this
				return this.companyTypes.filter(function (status) {
			...
			sortedcompanyType: function () {
				return this.filteredcompanyType.sort((a, b) => {
					let modifier = 1
