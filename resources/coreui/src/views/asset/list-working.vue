				...
				<b-card header="" border-variant="light">
					...
					<div class="row">
						...
						<div class="col-md-7 col-sm-12 text-right header-menu-nav">
							...
							<router-link v-if="$store.state.user.contact_type != 'Technician' && $store.state.user.contact_type != 'Manager'"  :to="{ path: '/asset/create' }" class="btn btn-link">
								<i class="fa fa-plus-square icon" title="Add A New Asset"></i>
							</router-link>
							
							<router-link v-if="$store.state.user.contact_type != 'Technician' && $store.state.user.contact_type != 'Manager'" :to="{ name: 'Upload Bulk Asset' }" class="btn btn-link">
								<i class="fa fa-upload icon" title="Upload Bulk Assets"></i>
							</router-link>
							<button v-if="$store.state.user.contact_type != 'Technician' && $store.state.user.contact_type != 'Manager'" :disabled="!selectedIds.length" class="btn btn-link" @click.prevent="erase" title="Delete Item">
								<i class="fa fa-trash icon" title="Delete Items"></i>
							</button>
							<router-link v-if="$store.state.user.contact_type != 'Technician' && $store.state.user.contact_type != 'Manager'" :to="{ path: '/write-off-assets' }" class="btn btn-link">
								<i class="fa fa-archive icon" title="See Write Off Assets"></i>
							</router-link>
							...
							<table class="table table-striped table-responsive">
								...
								<tbody>
									<tr v-for="(asset, index) in sortedAsset" :key="index">
										...
										<!-- <td v-if="checkColumnVisibilityStatus('created_at')">
											{{ asset.created_at }}
										</td> -->
										<td v-if="checkColumnVisibilityStatus('updated_at')">
											{{ asset.updated_at }}
										</td>
										<!-- <td v-if="checkColumnVisibilityStatus('created_name')">
											{{ asset.created_name }}
										</td>
										<td v-if="checkColumnVisibilityStatus('updated_name')">
											{{ asset.updated_name }}
										</td> -->
									</tr>
								</tbody>
		...
		data() {
			return {
				...
				columns: [
					...
					{name : 'date', label : 'Placement Date'},
				]
			}
		},
		created() {
		...
		computed: {
			filteredAsset: function () {
				...
				return this.assets.filter(function (asset) {
					...
					.filter(function (asset) {
						return asset.created_at ? asset.created_at.toString().indexOf(self.search.created_at.toString()) >= 0 : true
					})
					.filter(function (asset) {
						return asset.date ? asset.date.toString().indexOf(self.search.date.toString()) >= 0 : true
					})
					.filter(function (asset) {
						return asset.updated_at ? asset.updated_at.toString().indexOf(self.search.updated_at.toString()) >= 0 : true
					})
					.filter(function (asset) {
						return asset.updated_name ? asset.updated_name.toString().indexOf(self.search.updated_name.toString()) >= 0 : true
					})
					.filter(function (asset) {
						return asset.created_name ? asset.created_name.toString().indexOf(self.search.created_name.toString()) >= 0 : true
					})
			},
