                ...
                <form enctype="multipart/form-data" autocomplete='off' @submit.prevent="submitForm">
                    <div class="card border-0">
                        <div class="form-group form-actions">
                                <router-link :to="{ name: 'Job Priority List'}">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    <b-form-fieldset label="Priority Type">
                                        <b-form-input type="text" :class="errors.has('name') ? 'form-control is-invalid':'form-control'" v-validate="'required'"
                                            name="type" v-model="priority.name" placeholder="Please enter job priority"></b-form-input>
                                    </b-form-fieldset>
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    <b-form-textarea id="textarea1" v-model="priority.comment" placeholder="Comments" :rows="3" :max-rows="6">
                                    </b-form-textarea>
                                </div>
                            </div>
                            <!-- {{priority}} {{edit}} -->
...
<script>
    export default {
        ...
        created() {
            ...
            this.priority.account_id = this.$store.state.user.account_id;
        ...
        methods: {
            ...
            submitForm() {
                self = this;
                if (this.edit) {
                    this.priority.updated_by = this.$store.state.user.id;
                    this.$http
                        .put(`/api/priority`, this.priority,
                            {
                                headers: {
                    ...
                    this.priority.created_by = this.$store.state.user.id;
                    this.$http
                        .post(`/api/priority`, this.priority,
                            {
                                headers: {
                                    Authorization: `Bearer ${this.$store.state.token}`
                                }
