				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/priority/create' }" class="btn btn-link">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(priority, index) in sortedPriority" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="priority.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkColumnVisibilityStatus('name')">
											<router-link :to="{ path: `/priority/${priority.id}`}">
												{{ priority.name }}
											</router-link>
										</td>
									</tr>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				this.$swal('priority Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/priority/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedPriority.forEach(priority => {
							self.selectedIds.push(priority.id.toString())
						})
					}
					else {
			...
			load(page) {
				NProgress.start()
				let url = `/api/priorities-datatable`
		...
		computed: {
			filteredPriority: function () {
				...
				return this.priorities.filter(function (priority) {
					return priority.name ? priority.name.toLowerCase().indexOf(self.search.name.toLowerCase()) >= 0 : true
				})	
			},
