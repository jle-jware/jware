				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/roleType/create' }" class="btn btn-link">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(roleType, index) in sortedJobname" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="roleType.id">
											</b-form-checkbox>
										</td>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				this.$swal('roleType Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/roleType/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedJobname.forEach(roleType => {
							self.selectedIds.push(roleType.id.toString())
						})
					}
					else {
			...
			load(page) {
				NProgress.start()
				let url = `/api/roleTypes-datatable`
		...
		computed: {
			filteredJobname: function () {
				var self = this
				return this.roleTypes.filter(function (roleType) {
					return roleType.name ? roleType.name.toLowerCase().indexOf(self.search.name.toLowerCase()) >= 0 : true
				})	
			},
