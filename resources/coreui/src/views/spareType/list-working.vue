				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/spareType/create' }" class="btn btn-link">
							...
							<download-excel :data="sortedSpareType" class="btn">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(spareType, index) in sortedSpareType" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="spareType.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkColumnVisibilityStatus('type')">
											<router-link :to="{ path: `/spareType/${spareType.id}`}">
												{{ spareType.type }}
											</router-link>
										</td>
									</tr>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				this.$swal('spareType Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/spareType/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedSpareType.forEach(spareType => {
							self.selectedIds.push(spareType.id.toString())
						})
					}
					else {
						if (self.sortedSpareType.length == self.selectedIds.length) {
							self.selectedIds = []
						}
					}
				})
			...
			load(page) {
				NProgress.start()
				let url = `/api/spareTypes-datatable`
				...
				this.$http.get(url + params,
					...
					.then(({ data }) => {
						console.log('spareTypes', data)
						this.spareTypes = data.data
		...
		computed: {
			filteredSpareType: function () {
				var self = this
				return this.spareTypes.filter(function (spareType) {
					return spareType.type ? spareType.type.toLowerCase().indexOf(self.search.type.toLowerCase()) >= 0 : true
				})	
			},
			sortedSpareType: function () {
				return this.filteredSpareType.sort((a, b) => {
					let modifier = 1
