				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-12 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ name: 'Add Spare Type' }" class="btn btn-link">
							...
							<tr v-for="spareType in spareTypes" v-bind:key="spareType.id">
								<td>
									<router-link :to="{ path: `/spareType/${spareType.id}`}">
										<i class="fa fa-edit fa-lg" title="Edit Model Type" style="font-size:20px;  color: black;"></i>
									</router-link>

									<router-link :to="{ path:`/spareTypes`}">
										<span v-on:click="erase(spareType.id)">
											<i class="fa fa-trash fa-lg" title="Delete Model Type" style="font-size:20px;  color: black;"></i>
										</span>
									</router-link>
								</td>
								<td>
		...
		methods: {
			...
			erase(id) {
				...
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`api/spareType/${id}`, {
							headers: {
								Authorization: `Bearer ${this.$store.state.token}`
							}
						})
