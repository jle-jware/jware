                ...
                <form enctype="multipart/form-data" autocomplete='off' @submit.prevent="submitForm">
                    <div class="card border-0">
                        <div class="form-group form-actions">
                                <router-link :to="{ name: 'Job Status List'}">
                                ...
                                <div class="form-group col-sm-12">
                                    <b-form-fieldset label="Job Status">
                                        <b-form-input type="text" :class="errors.has('type') ? 'form-control is-invalid':'form-control'" v-validate="'required'"
                                            name="type" v-model="jobStat.type" placeholder="Please enter"></b-form-input>
                                    </b-form-fieldset>
                                </div>
                            </div>
                            <div class="row">
...
<script>
    export default {
        ...
        created() {
            ...
            this.jobStat.account_id = this.$store.state.user.account_id;
        ...
        methods: {
            ...
            submitForm() {
                ...
                self = this
                if (this.edit) {
                    this.jobStat.updated_by = this.$store.state.user.id
                    this.$http
                        .put(`/api/jobStat`, this.jobStat,
                            {
                                headers: {
                    ...
                    this.jobStat.created_by = this.$store.state.user.id
                    this.$http
                        .post(`/api/jobStat`, this.jobStat,
                            {
                                headers: {
                                    Authorization: `Bearer ${this.$store.state.token}`
                                }
