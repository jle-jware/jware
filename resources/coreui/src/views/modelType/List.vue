				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-12 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ name: 'Add Model Type' }" class="btn btn-link">
						...
						<tbody>
							<tr v-for="modelType in modelTypes" v-bind:key="modelType.id">
								<td>
									...
									<router-link :to="{ path:`/modelTypes`}">
										<span v-on:click="erase(modelType.id)">
		...
		methods: {
			...
			erase(id) {
				...
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/modelType/${id}`, {
							headers: {
								Authorization: `Bearer ${this.$store.state.token}`
							}
						})
