				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/assetStat/create' }" class="btn btn-link">
							...
							<download-excel :data="sortedAssetStat" class="btn">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(assetStat, index) in sortedAssetStat" :key="index">
										<td v-if="checkSelectedColumn('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="assetStat.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkSelectedColumn('type')">
											<router-link :to="{ path: `/assetStat/${assetStat.id}`}">
												{{ assetStat.type }}
											</router-link>
										</td>
										<td v-if="checkSelectedColumn('comment')">{{ assetStat.comment }}</td>
									</tr>
								</tbody>
								<!-- Table Body End -->
							</table>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				// Use sweetalret2
				this.$swal('Asset Status Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/assetStat/${this.selectedIds.join(',')}`, {
							headers: {
							...
							.catch(error => {
								if (error.response) {
									...
									if (error.response.assetStat === 401) {
										this.$router.push('/login')
									}
								}
							})
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedAssetStat.forEach(assetStat => {
							self.selectedIds.push(assetStat.id.toString())
						})
					}
					else {
						if (self.sortedAssetStat.length == self.selectedIds.length) {
							self.selectedIds = []
						}
					}
				})
			...
			load(page) {
				let url = `/api/assetStats-datatable?page=${page}&per_page=${this.pagination.per_page}&search=${this.search.global}`
				this.$http.get(url,
					{
						headers: {
							Authorization: `Bearer ${this.$store.state.token}`
					...
					.then(({ data }) => {
						console.log('assetStats', data)
						this.assetStats = data.data
		...
		computed: {
			filteredAssetStat: function () {
				var self = this
				return this.assetStats.filter(function (assetStat) {
					return assetStat.type ? assetStat.type.toLowerCase().indexOf(self.search.type.toLowerCase()) >= 0 : true
				})
					.filter(function (assetStat) {
						return assetStat.comment ? assetStat.comment.toLowerCase().indexOf(self.search.comment.toLowerCase()) >= 0 : true
					})
			},
			sortedAssetStat: function () {
				return this.filteredAssetStat.sort((a, b) => {
					let modifier = 1
