				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/inspectionResultType/create' }" class="btn btn-link">
							...
							<download-excel :data="sortedResult" class="btn">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(inspectionResultType, index) in sortedResult" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="inspectionResultType.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkColumnVisibilityStatus('name')">
											<router-link :to="{ path: `/inspectionResultType/${inspectionResultType.id}`}">
												{{ inspectionResultType.name }}
											</router-link>
										</td>
									</tr>
								</tbody>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				this.$swal('Result Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
				...
				}).then((inspectionResultType) => {
					if (inspectionResultType.value) {
						this.$http.delete(`/api/inspectionResultType/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedResult.forEach(inspectionResultType => {
							self.selectedIds.push(inspectionResultType.id.toString())
						})
					}
					else {
						if (self.sortedResult.length == self.selectedIds.length) {
							self.selectedIds = []
						}
					}
				})
			...
			load(page) {
				NProgress.start()
				let url = `/api/inspectionResultTypes-datatable`
				...
				this.$http.get(url + params,
					...
					.then(({ data }) => {
						console.log('inspectionResultTypes', data)
						this.inspectionResultTypes = data.data
		...
		computed: {
			filteredResult: function () {
				var self = this
				return this.inspectionResultTypes.filter(function (inspectionResultType) {
					return inspectionResultType.name ? inspectionResultType.name.toLowerCase().indexOf(self.search.name.toLowerCase()) >= 0 : true
				})	
			},
			sortedResult: function () {
				return this.filteredResult.sort((a, b) => {
					let modifier = 1
