				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/documentType/create' }" class="btn btn-link">
							...
							<download-excel :data="sortedDocumentType" class="btn">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(documentType, index) in sortedDocumentType" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="documentType.id">
											</b-form-checkbox>
										</td>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				this.$swal('documentType Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/documentType/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedDocumentType.forEach(documentType => {
							self.selectedIds.push(documentType.id.toString())
						})
					}
					else {
						if (self.sortedDocumentType.length == self.selectedIds.length) {
							self.selectedIds = []
						}
					}
				})
			...
			load(page) {
				NProgress.start()
				let url = `/api/documentTypes-datatable`
		...
		computed: {
			filteredDocumentType: function () {
				var self = this
				return this.documentTypes.filter(function (documentType) {
					return documentType.type ? documentType.type.toLowerCase().indexOf(self.search.type.toLowerCase()) >= 0 : true
				})	
			},
			sortedDocumentType: function () {
				return this.filteredDocumentType.sort((a, b) => {
					let modifier = 1
