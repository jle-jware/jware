				...
				<b-card header="" border-variant="light">
					<div class="row">
						...
						<div class="col-md-9 col-sm-12 text-right header-menu-nav">
							...
							<router-link :to="{ path: '/milestoneType/create' }" class="btn btn-link">
							...
							<download-excel :data="sortedMilestoneType" class="btn">
						...
						<div class="col-md-12">
							<table class="table table-striped table-responsive">
								...
								<!-- Table Body -->
								<tbody>
									<tr v-for="(milestoneType, index) in sortedMilestoneType" :key="index">
										<td v-if="checkColumnVisibilityStatus('all')">
											<b-form-checkbox v-model="selectedIds" @change="rowSelect" :value="milestoneType.id">
											</b-form-checkbox>
										</td>
										<td v-if="checkColumnVisibilityStatus('name')">
											<router-link :to="{ path: `/milestoneType/${milestoneType.id}`}">
												{{ milestoneType.name }}
											</router-link>
										</td>
									</tr>
...
<script>
	...
	export default {
		...
		methods: {
			showAlert() {
				this.$swal('milestoneType Deleted Successfully')
			},
			erase() {
				self = this
				self.$swal({
					...
					if (result.value) {
						this.$http.delete(`/api/milestoneType/${this.selectedIds.join(',')}`, {
							headers: {
			...
			allRowSelect() {
				...
				Vue.nextTick(function () {
					...
					if (self.all) {
						self.sortedMilestoneType.forEach(milestoneType => {
							self.selectedIds.push(milestoneType.id.toString())
						})
					}
					else {
						if (self.sortedMilestoneType.length == self.selectedIds.length) {
							self.selectedIds = []
						}
					}
				})
			...
			load(page) {
				NProgress.start()
				let url = `/api/milestoneTypes-datatable`
				...
				this.$http.get(url + params,
					...
					.then(({ data }) => {
						console.log('milestoneTypes', data)
						this.milestoneTypes = data.data
		...
		computed: {
			filteredMilestoneType: function () {
				var self = this
				return this.priorities.filter(function (milestoneType) {
					return milestoneType.name ? milestoneType.name.toLowerCase().indexOf(self.search.name.toLowerCase()) >= 0 : true
				})	
			},
			sortedMilestoneType: function () {
				return this.filteredMilestoneType.sort((a, b) => {
					let modifier = 1
